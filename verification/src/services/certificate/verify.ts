...
async function verifyCertificate(req: Request) {
	let { khasraNo, village, subDistrict, district, state, hash, hashType } = req.body

	if (!hash || !hashType || !isDataValid(khasraNo, village, subDistrict, district, state)) {
	...
	if (hashType == 'LAND') {
		const record = await queryLand(khasraNo, village, subDistrict, district, state)
		...
		const response = await queryOwnershipHistory(khasraNo, village, subDistrict, district, state)
...
function isDataValid(khasraNo: any, village: any, subDistrict: any, district: any, state: any) {
	const khasraRe = new RegExp('^[0-9]+(/[0-9]+)*$')
	const nameRe = new RegExp('^[a-zA-Z][a-zA-Z ]*$')

	if (!khasraRe.test(khasraNo) || !nameRe.test(village) || !nameRe.test(subDistrict) || !nameRe.test(district) || !nameRe.test(state))
		return false
